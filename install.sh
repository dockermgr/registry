...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/registry"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="registry"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/registry"
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [registry.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${REGISTRY_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$CONTAINER_USER_PASS" ]; then
  ...
  CONTAINER_USER_PASS="${REGISTRY_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
