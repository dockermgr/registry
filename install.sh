...
__host_name() { hostname -f 2>/dev/null | grep '\.' | grep '^' || hostname -f 2>/dev/null | grep '^' || echo "$APPNAME.$HOSTNAME"; }
...
SERVER_FULL_DOMAIN="${SET_HOSTNAME:-$(hostname -d 2>/dev/null | grep '^' || echo 'home')}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
